Link: https://yellowpaper.LQG.org/
Title: Yellow Paper

Abstract:
The LQG protocol is built on top of existing pool-based lending protocols, improving rates for both lenders and borrowers while
preserving the same liquidity and liquidation parameters. This paper
aims to provide a detailed description of the LQG protocol, notably
the innovative mechanisms that make it work, along with invariants
and theorems about it.

Mathis Gontier Delaunay co-founder of LQG Labs
Quentin Garchery employee of LQG Labs
Paul Frambot co-founder of LQG Labs
Merlin Egalite co-founder of LQG Labs
Julien Thomas co-founder of LQG Labs
Katia Babbar† from the University of Oxford

Acknowledgments
This work would not have been possible without the involvement of the rest
of the LQG Labs team and the advisors of the project, including but not
limited to Vincent Danos, Adrien Husson, Jean Krivine from the Mangrove
team, and a16z research.

This paper, referenced as the yellow paper, assumes that the reader is familiar with the general idea behind the LQG protocol. To strengthen their
intuition about the LQG protocol, the reader may first go through the
white paper
...
The LQG protocol, for its part, leverages the composability and liquidity of existing PLFs, to create efficient and liquid peer-to-peer markets of
supply and borrow positions with near-zero spread. The claim is that LQG strictly improves pool-based liquidity protocols from an individual user
perspective. In particular, LQG has the same liquidation parameters and
...
the LQG protocol and its unique mechanisms. Then, to demonstrate the
...
describes the on-chain environment that will be useful for LQG, including
the involved contracts’ functions. Section 3 details the structure and operation of LQG, giving us a basis to analyze and prove some of its relevant
...
supply, and aggregated peer-to-peer integrity, LQG’s positions integrity
...
to reason about the LQG protocol and its environment. We first detail
...
General description of LQG
In this section, we describe the main functionalities of the LQG protocol.
We start by detailing the internals of LQG: first, the storage variables of
...
lines evolve at pool rates, and LQG reuses the index mechanism of the
underlying pool to track them. The protocol also has its own peer-to-peer
scaled unit and associated index.

Suppliers have two balances in LQG. The pool supply balance sPool is in a unit that grows in underlying token value at r^S speed and represents the supply that is deposited on the pool. The matched peer-to-peer supply balance s^P2P grows at r
^P2P speed, and represents the matched peer-to-peer supply.

The balances separation is the mechanism that makes LQG positions not
...
It constantly grows with time, at a speed that depends on pool indexes evolution (see theorem 5.2.1), according to the interest rates model of LQG.
...
It is
helpful to explain what is the rate experienced by the users of LQG, but it
is not part of the storage. Indeed, LQG does not use the rates of the pool
to compute the indexes. Instead LQG is only relying on the indexes, which
...
LQG mirrors PLF’s five main functions: supply, withdraw, borrow, and
...
The supply^M function is used to deposit funds to the LQG protocol, which
starts accumulating interest at a potentially improved rate compared to the
underlying lending pool.

Peer-to-peer supply:
Promote borrowers: LQG matches the incoming liquidity
...
The borrow^M function is used to borrow funds from the LQG protocol,
potentially paying less interest compared to the underlying lending pool.

Peer-to-peer borrow:
Promote suppliers: LQG matches the incoming demand
...
1. Pool withdraw:
   ...
   on the pool, LQG withdraws the corresponding part of its position on the underlying pool. It favors this option to maximize the
...
2. Peer-to-peer withdraw:
   ...
   LQG replaces him with other pool suppliers. It favors this
   ...
   (b) Breaking withdraw:
   Demote borrowers: LQG breaks the peer-to-peer credit
...
The repay^M function is used to repay the accumulated debt to the LQG
protocol.

1. Pool repay:
   (a) No matching process: If some of the user’s debt is borrowed
   from the pool, LQG repays liquidity to the underlying pool. It
...
2. Peer-to-peer repay:
   ...
   LQG replaces the peer-to-peer credit lines with other pool
   ...
   on the pool.
   (b) Breaking repay:
   i. Demote suppliers: LQG breaks the peer-to-peer credit
   lines of the repaying user with the remaining matched suppliers and deposits the unmatched liquidity on the underlying
   pool. The matching engine unmatches all the remaining liquidity from peer-to-peer suppliers. LQG supply on the
   underlying pool to reconnect the suppliers to the pool.

The liquidation on LQG works the same way as liquidation on the underlying pool. When a position of a user is not sufficiently collateralized,
a liquidator can repay part of the debt of the user and seize a part of the
collateral of the user.

LQG uses the same values for the collateral factors F, closeFactor, and
...
The non-liquidation theorem states that the position of LQG on the
...
fraction, in value, of the supplied assets. Then, because LQG enforces that
...
against the pool supply, which gives us the position of LQG itself on the
...
The non liquidation theorem is crucial for LQG.

Matching engine
The role of the matching engine is to create and break the peer-to-peer credit
lines of users. In this section, we will first explain how the LQG protocol
...
In this section, we will go through all the LQG mechanisms related to
interest rates. To this end, we first define the rates that we will consider
and then explain the different methods used to update the indexes, including LQG’s approach. We conclude the section with a proof of the rate
improvement that LQG is achieving

Pool rates:
LQG’s users that are not matched peer-to-peer are connected to the pool
...
While users on the pool necessarily earn and pay the exact pool rates, LQG
...
users and give it back to the LQG DAO. In this section we focus on the
impact the fee has on LQG’s rates, an explanation of the fee’s accounting
...
However, please note that LQG is currently not taking any fee.

More information are provided into the yellowpaper.
We show in this subsection that LQG rates are improved compared to the
...
we already have the pool fallback: LQG fills the rest of the order with a
...
breaking withdraw and breaking repay) by increasing the position of LQG
...
peer-to-peer users. It introduces a difference19 between the peer-to-peer liquidity evolving at LQG’s rates and the sum of peer-to-peer balances of
...
engine cannot find enough liquidity in this step, then LQG borrows (resp.
...
have a bounded gas cost, we know that LQG’s operations’ overall gas
costs are bounded. This way, LQG can scale as more users enter the
different markets.
The matching engine is not mandatory for operations to succeed. Thus we
introduce the possibility for LQG users to choose the maxGasForMatching
...
The LQG protocol is an improvement of the established protocols for
...
LQG, explained in section 3, is to improve capital efficiency by matching
...
we prove that LQG users enjoy improved rates compared to users of the
